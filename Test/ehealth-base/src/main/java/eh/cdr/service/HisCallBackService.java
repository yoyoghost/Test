    ...
    public static void checkPassSuccess(RecipeCheckPassResult result, boolean isCheckPass) {
        ...
        if(isCheckPass){
            ...
            if(recipe.canMedicalPay()){
                ...
                attrMap.put("giveMode", RecipeConstant.GIVEMODE_SEND_TO_HOME);
            }
                ...
                if (null != recipe) {
                    ...
                    if (null != recipeId) {
                        ...
                        if(beforeStatus == RecipeStatusConstant.HAVE_PAY){
                            logger.info("havePayRecipesFromHis 处方ID："+recipeId+" 已是已支付状态，无需更新");
                            continue;
                        }
                        if(null == recipe.getPayDate()){
                            attrMap.put("payDate", new DateTime().toDate());
                        }
                        if(null == recipe.getGiveMode()){
                            attrMap.put("giveMode", RecipeConstant.GIVEMODE_TO_HOS);
                        }
                        if(null == recipe.getPayMode()){
                            attrMap.put("payMode", RecipeConstant.PAYMODE_TO_HOS);
                        }
