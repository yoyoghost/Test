...
import eh.cdr.drugsenterprise.ThirdEnterpriseCallService;
    ...
    public HosRecipeResult platformRecipeCreate(List<HospitalRecipeBean> hospitalRecipeList){
        ...
        for(HospitalRecipeBean hospitalRecipe : hospitalRecipeList) {
            ...
            if (null != recipe) {
                ...
                ThirdEnterpriseCallService takeDrugService = AppContextHolder.getBean("eh.takeDrugService", ThirdEnterpriseCallService.class);

                Integer originClinicOrgan = recipe.getOriginClinicOrgan();
                String organName = organDAO.getNameById(originClinicOrgan);
                ...
                //设置医生信息(开方医生，审核医生)
                Employment employment = employmentDAO.getByJobNumberAndOrganId(hospitalRecipe.getDoctorID(), originClinicOrgan);
                if (null != employment) {
                    ...
                    if (StringUtils.isNotEmpty(ysJobNumber)) {
                        Employment ysEmployment = employmentDAO.getByJobNumberAndOrganId(ysJobNumber, originClinicOrgan);
                    ...
                    //设置患者信息
                    try {
                        Patient patient = patientDAO.getOrUpdate(hospitalRecipe.convertNgariPatient());
                        if (null == patient) {
                            result.setMsgCode(HosRecipeResult.FAIL);
                    ...
                    if (null != recipeId) {
                        ...
                        orderMap.put("payway", PayWayEnum.UNKNOW.getCode());
                        ...
                        if (null != orderCreateResult && OrderCreateResult.SUCCESS.equals(orderCreateResult.getCode())) {
                            ...
                            if (RecipeConstant.PAYMODE_TFDS.equals(recipe.getPayMode())) {
                                Organ subOrgan = organDAO.get(recipe.getClinicOrgan());
                                //到店取药则自动完成用户确认操作
                                ...
                                Map<String, Object> backMap = takeDrugService.userConfirm(paramMap);
                                if (null != backMap && ThirdEnterpriseCallService.REQUEST_OK.equals(MapValueUtil.getInteger(backMap, "code"))) {
...
//                                                    sendSubOrganHisRecipe(recipeId, hospitalRecipe.getSubOrganId());
                                    ...
                                    result.setMsgCode(HosRecipeResult.FAIL);
                                    result.setMsg(prefix+"订单用户确认失败,原因：" + MapValueUtil.getString(backMap, "msg"));
                                }
                            }
                        } else {
                            //删除处方
    ...
    public HosRecipeResult platformRecipeStatusUpdate(HospitalRecipeBean hospitalRecipe){
        ...
        if("0".equals(recipeStatus)){
            ...
            Map<String, Object> backMap = takeDrugService.recordDrugStoreResult(paramMap);
            if (null != backMap && ThirdEnterpriseCallService.REQUEST_OK.equals(MapValueUtil.getInteger(backMap, "code"))) {
                //成功
                logger.info("platformRecipeStatusUpdate 处理成功. recipeStatus=[{}]", recipeStatus);
            } else {
                result.setMsgCode(HosRecipeResult.FAIL);
                result.setMsg("平台处方状态更新失败,原因：" + MapValueUtil.getString(backMap, "msg"));
            }
    ...
    public HosRecipeResult platformRecipeSearch(HospitalSearchBean seachBean){
        ...
        Patient patient = patientDAO.getByIdCard(seachBean.getCertID());
        if(null == patient){
            result.setMsgCode(HosRecipeResult.FAIL);
        ...
        if(StringUtils.isNotEmpty(seachBean.getDoctorID())){
            Employment employment = employmentDAO.getByJobNumberAndOrganId(seachBean.getDoctorID(),Integer.valueOf(seachBean.getOrganID()));
            if(null != employment){
            ...
            for(Recipe recipe : dbList){
                ...
                try {
                    RecipeResultBean _result = hospitalRecipeBean.parseRecipe(recipe,recipeDetailDAO.findByRecipeId(recipe.getRecipeId()),
                            patient,healthCardDAO.findByCardOrganAndMpiId(recipe.getClinicOrgan(),recipe.getMpiid()));
