...
import eh.entity.cdr.RecipeCheck;
...
public abstract class RecipeCheckDAO extends HibernateSupportDelegateDAO<RecipeCheck> {
     ...
     * @return
     */
    public List<Recipe> searchRecipe(final Set<Integer> organs, final String searchString, final Integer searchFlag, final Integer start, final Integer limit){
        HibernateStatelessResultAction<List<Recipe>> action = new AbstractHibernateStatelessResultAction<List<Recipe>>() {
            @Override
            public void execute(StatelessSession ss) throws Exception {
                StringBuilder hql = new StringBuilder("select distinct r from Recipe r");
                if(0 == searchFlag){
                    hql.append(", Doctor d where r.doctor = d.doctorId and d.name like:searchString ");
                }
                else if(1 == searchFlag){
                    hql.append(", Doctor d where r.checker = d.doctorId and d.name like:searchString ");
                }
                else if(2 == searchFlag){
                    hql.append(", Patient p where p.mpiId = r.mpiid and p.patientName like:searchString ");
                }
