...
public class HospitalRecipeBean implements Serializable{
    ...
    public Recipe convertNgariRecipe(){
        ...
        if("2".equals(this.deliveryType)){
            recipe.setGiveMode(RecipeConstant.GIVEMODE_TFDS);
            recipe.setPayMode(RecipeConstant.PAYMODE_TFDS);
        }else{
            recipe.setGiveMode(RecipeConstant.GIVEMODE_TO_HOS);
            recipe.setPayMode(RecipeConstant.PAYMODE_TO_HOS);
        }
        ...
        recipe.setFromflag(0);//从医院HIS获取的处方
        return recipe;
    }
    ...
    public List<Recipedetail> convertNgariDetail(){
        ...
        if(CollectionUtils.isNotEmpty(organDrugList)){
            ...
            Map<Integer, String> codeIdRel = new HashMap<>();//某机构内，药品编码与药品ID关系
            Map<String, BigDecimal> codePriceRel = new HashMap<>();//某机构内，药品编码与药品单价关系
            ...
            if(CollectionUtils.isNotEmpty(drugIds)){
                ...
                if(CollectionUtils.isNotEmpty(drugList)){
                    Map<String,DrugList> drugRel = new HashMap<>();//某机构内，药品编码与药品详情关系
    ...
    public Patient convertNgariPatient(){
     ...
     * @param patient
     */
    public RecipeResultBean parseRecipe(Recipe recipe, List<Recipedetail> details, Patient patient, List<HealthCard> cards){
