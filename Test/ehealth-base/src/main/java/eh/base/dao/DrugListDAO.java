...
import elastic.client.ESClient;
...
public abstract class DrugListDAO extends HibernateSupportDelegateDAO<DrugList>
    ...
    public List<DrugList> searchDrugListWithES(Integer organId, Integer drugType, String drugName,
        ...
        String organStr = null == organId?"":String.valueOf(organId);
        String drugTypeStr = null == drugType?"":String.valueOf(drugType);

        logger.info("Enter searchDrugListWithES organStr = " + organStr +
                ", drugTypeStr = " + drugTypeStr + ", drugName = " + drugName);
        Map map = Maps.newHashMap();
        //map中的两个值 是对应的数据库和表名
        map.put("index", PropertyUtil.getPropValue("es.index"));
        map.put("type","drug");
        /*List<String> list = ESClient.searchPages(map, drugName, organStr, drugTypeStr, start, limit);*/
        List<String> list = null;

        // 前端搜索药名的时候统一转成消息传给搜索引擎
        drugName = drugName.toLowerCase();
        try
        {
            list = ESClient.searchHighlightedPages(map, drugName, organStr, drugTypeStr, start, limit);
        }
        catch (Exception e)
        {
            logger.error(
                "searchDrugListWithES searchHighlightedPagesForDoctor error={}",
                e.getMessage());
        }
        List<DrugList> dList = new ArrayList<>();
        ...
        if(CollectionUtils.isNotEmpty(list))
        {
            for (String s : list)
            {
                ...
                {
                    logger.error("searchDrugListWithES JSONUtils.parse error! String = " + s);
                }
            ...
            CdrUtil.getHospitalPrice(dList);
        }
        else
        {
        ...
        HibernateStatelessResultAction<List<DrugList>> action = new AbstractHibernateStatelessResultAction<List<DrugList>>() {
            ...
            public void execute(StatelessSession ss) throws DAOException {
                StringBuffer hql = new StringBuffer(
                        "select d From DrugList d, OrganDrugList o, Organ og where o.organId = og.organId and " +
                                "og.type = 1 and og.status = 1 and d.status=1 and o.status=1 and d.drugId=o.drugId ");
    ...
    public abstract Long getEffDrugNum(@DAOParam("organId") int organId);

    /**
     * 供 employmentdao-findEffEmpWithDrug 调用
     * 获取中药药品数量
     */
    @DAOMethod(sql = "select count(*) From DrugList d,OrganDrugList o where o.organId=:organId and d.status=1 and o.status=1 and d.drugId=o.drugId and d.drugType=3")
    public abstract Long getChineseMedicineNum(@DAOParam("organId") int organId);
    /**
    ...
    public abstract List<DrugList> findDrugsByOrganId(@DAOParam("organId") Integer organId);
